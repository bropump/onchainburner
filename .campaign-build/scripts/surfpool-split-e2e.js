...
exports.JUPITER_PROGRAM = exports.RPC_URL = exports.TOKENS = exports.SPLIT_DISCRIMINATOR = exports.QUOTE_AUTHORITY = exports.PROGRAM = exports.FORK_ROUTABLE_DEXES = exports.ERROR_NAMES = exports.DEFAULT_MAX_ACCOUNTS_PER_LEG = exports.BPS_TOTAL = void 0;
exports.buildSplitInstruction = buildSplitInstruction;
exports.deriveSplitPda = deriveSplitPda;
exports.ensureSharedLookupTable = ensureSharedLookupTable;
exports.ensurePumpVolumeAccumulators = ensurePumpVolumeAccumulators;
exports.ensurePumpCreatorVaultsFunded = ensurePumpCreatorVaultsFunded;
exports.ensureBondingCurvesMigrated = ensureBondingCurvesMigrated;
exports.extendBondingCurveIx = extendBondingCurveIx;
exports.initUserVolumeAccumulatorIx = initUserVolumeAccumulatorIx;
exports.createVaultLookupTable = createVaultLookupTable;
exports.fetchJson = fetchJson;
exports.getLookupTables = getLookupTables;
exports.prepareLegs = prepareLegs;
exports.readPayer = readPayer;
exports.readQuoteAuthority = readQuoteAuthority;
exports.runSplitCase = runSplitCase;
exports.sendInstructions = sendInstructions;
exports.solToLamports = solToLamports;
exports.splitAmounts = splitAmounts;
exports.splitInstructionData = splitInstructionData;
exports.customErrorCode = customErrorCode;
exports.attributeFailure = attributeFailure;
...
const PROGRAM = new web3_js_1.PublicKey("5kTgbKKDWTcyPoEp2S5Lunz1vsSLN92CzwNis4GQhnkV");
exports.PROGRAM = PROGRAM;
const JUPITER_PROGRAM = new web3_js_1.PublicKey("JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4");
exports.JUPITER_PROGRAM = JUPITER_PROGRAM;
const JUPITER_EVENT_AUTHORITY = new web3_js_1.PublicKey("D8cy77BBepLMngZx6ZukaTff5hCt1HrWyKk3Hnd9oitf");
const ROUTE_V2_DISCRIMINATOR = "bb64facc31c4af14";
const SHARED_ACCOUNTS_ROUTE_V2_DISCRIMINATOR = "d19853937cfed8e9";
const QUOTE_AUTHORITY = new web3_js_1.PublicKey("Afs7CEFqSHNXFo5r6XhUmGYAzhC7qnKKA1UPUMU8quz8");
exports.QUOTE_AUTHORITY = QUOTE_AUTHORITY;
...
const TOKENS = {
    NEIRO: new web3_js_1.PublicKey("CTg3ZgYx79zrE1MteDVkmkcGniiFrK1hJ6yiabropump"),
    PUMP: new web3_js_1.PublicKey("pumpCmXqMfrsAkQ5r49WcJnRayYRqmXz6ae8H7H9Dfn"),
    ...
    WIF: new web3_js_1.PublicKey("EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm"),
    FARTCOIN: new web3_js_1.PublicKey("9BB6NFEcjBCtnNLFko2FqVQBq8HHM13kCyYcdQbgpump"),
    POPCAT: new web3_js_1.PublicKey("7GCihgDB8fe6KNjn2MYtkzZcRjQy3t9GHdC8uHYmW2hr"),
...
const FORK_ROUTABLE_DEXES = process.env.FORK_DEXES
    ...
    : process.env.FORK_DEX_PROFILE === "pool"
        ...
        : [];
exports.FORK_ROUTABLE_DEXES = FORK_ROUTABLE_DEXES;
/**
 ...
 */
const DEFAULT_MAX_ACCOUNTS_PER_LEG = {
    1: undefined,
    2: 26,
    3: 16,
    4: 12,
};
exports.DEFAULT_MAX_ACCOUNTS_PER_LEG = DEFAULT_MAX_ACCOUNTS_PER_LEG;
// ---------------------------------------------------------------------------
// Key / RPC plumbing
// ---------------------------------------------------------------------------
function readPayer() {
    const filename = process.env.SOLANA_KEYPAIR ??
        node_path_1.default.join(node_os_1.default.homedir(), ".config", "solana", "id.json");
    return web3_js_1.Keypair.fromSecretKey(Uint8Array.from(JSON.parse(node_fs_1.default.readFileSync(filename, "utf8"))));
}
function readQuoteAuthority() {
    const filename = process.env.QUOTE_AUTHORITY_KEYPAIR ??
        node_path_1.default.join(node_os_1.default.homedir(), ".config", "solana", "onchain-burner-quote-authority.json");
    const authority = web3_js_1.Keypair.fromSecretKey(Uint8Array.from(JSON.parse(node_fs_1.default.readFileSync(filename, "utf8"))));
    if (!authority.publicKey.equals(QUOTE_AUTHORITY)) {
        throw new Error(`quote authority ${authority.publicKey.toBase58()} != ${QUOTE_AUTHORITY.toBase58()}`);
    }
    return authority;
}
async function fetchJson(url, init, returnBadRequestJson = false) {
    ...
    for (let attempt = 0; attempt < 7; attempt += 1) {
        try {
            ...
            if (!response.ok) {
                throw Object.assign(new Error(`${response.status} ${await response.text()}`), {
                    fatal: true,
                });
            }
            return (await response.json());
...
function customErrorCode(error) {
    ...
    const match = text.match(/"Custom":\s*(\d+)/) ??
 ...
 */
function attributeFailure(logs, fallback) {
    for (const line of logs ?? []) {
        const failed = line.match(/^Program (\S+) failed: custom program error: 0x([0-9a-f]+)/i);
        if (failed) {
            return {
 ...
 */
function deriveSplitPda(launchMint, legs) {
...
function splitInstructionData(totalAmountIn, legs) {
    const header = Buffer.alloc(12);
    header.writeBigUInt64LE(totalAmountIn, 0);
    header.writeUInt32LE(legs.length, 8);
    const parts = [SPLIT_DISCRIMINATOR, header];
...
// ---------------------------------------------------------------------------
async function sendInstructions(connection, payer, label, instructions, extraSigners = []) {
...
async function getLookupTables(connection, addresses) {
    return (await Promise.all(addresses.map(async (address) => (await connection.getAddressLookupTable(new web3_js_1.PublicKey(address))).value))).filter((table) => table !== null);
}
let sharedLookupTable;
async function extendAndFreezeLookupTable(connection, payer, label, addresses) {
    const recentSlot = await connection.getSlot("confirmed");
    const [createIx, lookupTable] = web3_js_1.AddressLookupTableProgram.createLookupTable({
...
async function ensureSharedLookupTable(connection, payer) {
    if (sharedLookupTable)
        return sharedLookupTable;
    sharedLookupTable = await extendAndFreezeLookupTable(connection, payer, "shared-alt", [
        PROGRAM,
        JUPITER_PROGRAM,
 ...
 */
async function createVaultLookupTable(connection, payer, label, pda, wsolAta, launchMint, legs) {
 ...
 * introduced.
 */
function initUserVolumeAccumulatorIx(program, payer, user) {
    const [accumulator] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("user_volume_accumulator"), user.toBuffer()], program);
    const [eventAuthority] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("__event_authority")], program);
 ...
 */
function extendBondingCurveIx(payer, bondingCurve) {
    const [eventAuthority] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("__event_authority")], PUMP_FUN_PROGRAM);
    return new web3_js_1.TransactionInstruction({
 ...
 * token) is skipped.
 */
async function ensureBondingCurvesMigrated(connection, payer, mints) {
    const migrated = [];
    const instructions = [];
    for (const mint of mints) {
        const [curve] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("bonding-curve"), mint.toBuffer()], PUMP_FUN_PROGRAM);
    ...
    if (instructions.length) {
        await sendInstructions(connection, payer, "extend-bonding-curves", instructions);
    }
    return migrated;
}
async function ensurePumpVolumeAccumulators(connection, payer, vault) {
    const created = [];
    const instructions = [];
    for (const [name, program] of [
        ...
        const [accumulator] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("user_volume_accumulator"), vault.toBuffer()], program);
        if (await connection.getAccountInfo(accumulator, "confirmed"))
            continue;
        instructions.push(initUserVolumeAccumulatorIx(program, payer.publicKey, vault));
        created.push(name);
    }
    if (instructions.length) {
        await sendInstructions(connection, payer, "init-pump-volume-accumulators", instructions);
    }
    return created;
}
/** Mirror the program's exact Pump-credit admission for result accounting. */
async function snapshotPumpLamportCredits(connection, vault, legs) {
    const writableRouteAccounts = new Set(legs
    ...
    for (const program of [PUMP_FUN_PROGRAM, PUMP_SWAP_PROGRAM]) {
        ...
        if (!writableRouteAccounts.has(address.toBase58()))
            continue;
 ...
 */
async function ensurePumpCreatorVaultsFunded(connection, payer, legs) {
    ...
    for (const leg of legs) {
        ...
        const [curve] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("bonding-curve"), leg.mint.toBuffer()], PUMP_FUN_PROGRAM);
        ...
        const creator = new web3_js_1.PublicKey(curveInfo.data.subarray(49, 81));
        const [creatorVault] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("creator-vault"), creator.toBuffer()], PUMP_FUN_PROGRAM);
        if (!leg.routeAccounts.some((account) => account.isWritable && account.pubkey.equals(creatorVault))) {
            throw new Error(`Pump route omitted derived creator vault ${creatorVault.toBase58()}`);
        }
        const info = await connection.getAccountInfo(creatorVault, "confirmed");
        if (info &&
            (!info.owner.equals(web3_js_1.SystemProgram.programId) || info.data.length !== 0)) {
            throw new Error(`invalid Pump creator vault ${creatorVault.toBase58()}`);
        }
        const current = info?.lamports ?? 0;
        if (current >= floor || topUps.has(creatorVault.toBase58()))
            continue;
        topUps.set(creatorVault.toBase58(), web3_js_1.SystemProgram.transfer({
 ...
 * never mutated.
 */
function pumpRawCurveTrackVolumeOffset(data) {
    if (data.length !== 40)
        return undefined;
    if (data.subarray(0, 8).toString("hex") !== ROUTE_V2_DISCRIMINATOR) {
        return undefined;
    }
    if (data.readUInt32LE(30) !== 1)
        return undefined; // exactly one route step
    if (data[34] !== PUMP_RAW_CURVE_ROUTE_V2_VARIANT)
        return undefined;
    if (data[35] !== 0 && data[35] !== 1)
        return undefined; // bool track_volume
    if (data.readUInt16LE(36) !== BPS_TOTAL)
        return undefined; // 100% one hop
    if (data[38] !== 0 || data[39] !== 1)
        return undefined; // in/out indexes
    return 35;
}
/**
 ...
 */
async function ensureBareSystemPumpAccumulator(connection, payer, vault) {
    const [accumulator] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("user_volume_accumulator"), vault.toBuffer()], PUMP_FUN_PROGRAM);
    const rent = await connection.getMinimumBalanceForRentExemption(PUMP_USER_VOLUME_ACCUMULATOR_SPACE);
    const info = await connection.getAccountInfo(accumulator, "confirmed");
    if (info && !info.owner.equals(web3_js_1.SystemProgram.programId)) {
...
// ---------------------------------------------------------------------------
function assertV2Build(build, pda, wsolAta, targetMint, targetAta, targetTokenProgram, amountIn) {
    if (build.error)
        throw new Error(`Jupiter V2 build failed: ${build.error}`);
    if (build.swapMode !== "ExactIn" || BigInt(build.inAmount) !== amountIn) {
        throw new Error(`Jupiter V2 build changed the authorized input: mode=${build.swapMode} amount=${build.inAmount}`);
    }
    ...
    if (data.readBigUInt64LE(offsets.input) !== amountIn) {
        throw new Error(`Jupiter V2 instruction input does not equal the authorized input`);
    }
    if (data.readUInt16LE(offsets.platformFee) !== 0 ||
    ...
    const expected = [
        ...
        [offsets.sourceMint, spl_token_1.NATIVE_MINT],
        [offsets.destinationMint, targetMint],
        [offsets.sourceTokenProgram, spl_token_1.TOKEN_PROGRAM_ID],
    ...
    if (!shared)
        expected.push([7, targetAta]);
    for (const [index, key] of expected) {
        if (accounts[index].pubkey !== key.toBase58()) {
            throw new Error(`Jupiter V2 account ${index} is ${accounts[index].pubkey}, expected ${key.toBase58()}`);
        }
    }
    const extraSigners = accounts.filter((account) => account.isSigner && account.pubkey !== pda.toBase58());
    if (extraSigners.length) {
        throw new Error(`Jupiter V2 route needs signatures the burner withholds: ${extraSigners
            .map((account) => account.pubkey)
            .join(",")}`);
    }
    return shared ? "shared_accounts_route_v2" : "route_v2";
}
function buildLookupTables(build) {
    return Object.entries(build.addressesByLookupTableAddress ?? {}).map(([key, addresses]) => new web3_js_1.AddressLookupTableAccount({
        key: new web3_js_1.PublicKey(key),
        state: {
            deactivationSlot: BigInt("18446744073709551615"),
            lastExtendedSlot: 0,
            lastExtendedSlotStartIndex: 0,
            addresses: addresses.map((address) => new web3_js_1.PublicKey(address)),
        },
    }));
}
async function prepareLegs(connection, payer, pda, wsolAta, legs, total, slippageBps, maxAccountsPerLeg) {
    const amounts = splitAmounts(total, legs.map((leg) => leg.bps));
    const prepared = [];
    ...
    for (const [index, leg] of legs.entries()) {
        ...
        const ata = (0, spl_token_1.getAssociatedTokenAddressSync)(leg.mint, pda, true, tokenProgram);
        const url = new URL(`${JUPITER_API}/build`);
        url.searchParams.set("inputMint", spl_token_1.NATIVE_MINT.toBase58());
        ...
        let swap = await fetchJson(url.toString(), undefined, true);
        ...
        if (swap.error && maxAccountsPerLeg) {
            ...
            swap = await fetchJson(url.toString(), undefined, true);
        }
        const routeKind = assertV2Build(swap, pda, wsolAta, leg.mint, ata, tokenProgram, amounts[index]);
        // Only idempotent ATA creation may be pre-run; anything else could move
        // burner funds outside the atomic instruction.
        const unexpected = (swap.setupInstructions ?? []).filter((ix) => ix.programId !== spl_token_1.ASSOCIATED_TOKEN_PROGRAM_ID.toBase58() ||
            ix.data !== "AQ==");
        if (unexpected.length ||
            swap.cleanupInstruction ||
            (swap.otherInstructions ?? []).length ||
            swap.tipInstruction) {
            throw new Error(`unsafe auxiliary instructions for ${leg.label}: ${JSON.stringify({
                unexpected,
        ...
        for (const ix of swap.setupInstructions ?? []) {
            ...
            setupInstructions.push(new web3_js_1.TransactionInstruction({
        ...
        const routeKeys = swap.swapInstruction.accounts.map((a) => new web3_js_1.PublicKey(a.pubkey));
        const routeInfos = await connection.getMultipleAccountsInfo(routeKeys, "confirmed");
        for (const [index, info] of routeInfos.entries()) {
            ...
            for (const [candidateMint, candidateProgram] of [
                ...
                for (const owner of routeKeys) {
                    let derived;
                    try {
                        derived = (0, spl_token_1.getAssociatedTokenAddressSync)(candidateMint, owner, true, candidateProgram);
                    }
                    catch {
                        continue;
                    }
                    if (!derived.equals(missing))
                        continue;
                    setupInstructions.push((0, spl_token_1.createAssociatedTokenAccountIdempotentInstruction)(payer.publicKey, missing, owner, candidateMint, candidateProgram));
                    matched = true;
                    break;
                }
                if (matched)
        ...
        const pumpRawCurveFlagOffset = routeKind === "route_v2" &&
        ...
        prepared.push({
            ...
            minimumOutput,
            routeAccounts: swap.swapInstruction.accounts.map((a) => ({
                pubkey: new web3_js_1.PublicKey(a.pubkey),
                isSigner: false,
                isWritable: a.isWritable,
            })),
            ...
            routeLabel: `${routeKind}:${(swap.routePlan ?? [])
                ...
                .filter(Boolean)
                .join(">")}`,
            routePriceImpactPct: swap.priceImpactPct === undefined ? undefined : String(swap.priceImpactPct),
            pumpRawCurveFlagOffset,
        });
    }
    if (setupInstructions.length) {
        await sendInstructions(connection, payer, "jupiter-route-account-setup", setupInstructions);
    }
    return prepared;
}
function buildSplitInstruction(payer, quoteAuthority, pda, wsolAta, launchMint, legs, total, overrides = {}) {
    let keys = [
        { pubkey: payer, isSigner: true, isWritable: false },
        { pubkey: quoteAuthority, isSigner: true, isWritable: false },
    ...
    if (overrides.accountTamper)
        keys = overrides.accountTamper(keys);
    let data = splitInstructionData(total, legs.map((leg, index) => ({
        bps: overrides.bpsOverride?.[index] ?? leg.bps,
        minimumOutput: overrides.minimumOutputOverride?.[index] ?? leg.minimumOutput,
        routeAccountCount: leg.routeAccounts.length,
        jupiterData: leg.jupiterData,
    })));
    if (overrides.dataTamper)
        data = overrides.dataTamper(data);
    return new web3_js_1.TransactionInstruction({ programId: PROGRAM, keys, data });
}
async function runSplitCase(connection, payer, quoteAuthority, name, launchMint, legs, totalSol, options = {}) {
    ...
    try {
        ...
        const wsolAta = (0, spl_token_1.getAssociatedTokenAddressSync)(spl_token_1.NATIVE_MINT, pda, true, spl_token_1.TOKEN_PROGRAM_ID);
        // The launcher creates the vault's ATAs once and pays their rent; the
        // program itself never creates an account.
        const ataIxs = [
            (0, spl_token_1.createAssociatedTokenAccountIdempotentInstruction)(payer.publicKey, wsolAta, pda, spl_token_1.NATIVE_MINT, spl_token_1.TOKEN_PROGRAM_ID),
        ];
        for (const leg of legs) {
            const info = await connection.getAccountInfo(leg.mint, "confirmed");
            if (!info)
                throw new Error(`mint ${leg.label} missing on fork`);
            ataIxs.push((0, spl_token_1.createAssociatedTokenAccountIdempotentInstruction)(payer.publicKey, (0, spl_token_1.getAssociatedTokenAddressSync)(leg.mint, pda, true, info.owner), pda, leg.mint, info.owner));
        }
        ...
        summary.bondingCurvesMigrated = options.skipPumpAccumulator
            ? []
            : await ensureBondingCurvesMigrated(connection, payer, legs.map((l) => l.mint));
        ...
        const requestedMaxAccounts = options.maxAccountsPerLeg === 0
            ? undefined
            : options.maxAccountsPerLeg ??
                DEFAULT_MAX_ACCOUNTS_PER_LEG[legs.length];
        let prepared = await prepareLegs(connection, payer, pda, wsolAta, legs, total, 
        ...
        options.slippageBps ??
            (process.env.FORK_SLIPPAGE_BPS
                ...
                : undefined), requestedMaxAccounts);
        summary.pumpCreatorVaultsFunded = await ensurePumpCreatorVaultsFunded(connection, payer, prepared);
        ...
        const vaultLookupTable = prepared.length > 1
            ? await createVaultLookupTable(connection, payer, name, pda, wsolAta, launchMint, prepared)
            : undefined;
        ...
        const compileBurn = async (candidate) => {
            const instruction = buildSplitInstruction(caller.publicKey, quoteAuthority.publicKey, pda, wsolAta, launchMint, candidate, total, options.overrides);
            const lookupTables = [
                ...new Map([
            ...
            const transaction = new web3_js_1.VersionedTransaction(message);
            // The quote authority may be a REMOTE signer (Google Cloud KMS in the
            // production rehearsal), which has no local secret key and therefore
            // cannot go through `VersionedTransaction.sign`. Such a signer exposes
            // `signMessage(bytes) -> 64-byte signature`; its signature is placed at
            // the index its pubkey occupies, exactly as `sign` would.
            const remote = quoteAuthority.signMessage;
            if (remote) {
                transaction.sign([caller]);
                const index = message.staticAccountKeys.findIndex((key) => key.equals(quoteAuthority.publicKey));
                if (index < 0) {
                    throw new Error("quote authority is not a signer on this message");
                }
                transaction.signatures[index] = Buffer.from(remote(transaction.message.serialize()));
            }
            else {
                transaction.sign([caller, quoteAuthority]);
            }
            let bytes;
            try {
        ...
        const fittingCaps = [40, 32, 26, 20, 16, 12].filter((cap) => requestedMaxAccounts === undefined || cap < requestedMaxAccounts);
        for (const cap of fittingCaps) {
            if (compiled.bytes <= MAX_TRANSACTION_BYTES)
                break;
            prepared = await prepareLegs(connection, payer, pda, wsolAta, legs, total, options.slippageBps ??
                (process.env.FORK_SLIPPAGE_BPS
                    ? Number(process.env.FORK_SLIPPAGE_BPS)
                    : undefined), cap);
        ...
        let pumpCreditSnapshots = await snapshotPumpLamportCredits(connection, pda, prepared);
        ...
        // same caps.
        const exhaustedCompute = (result) => typeof result.value.err === "string"
            ? result.value.err === "ComputationalBudgetExceeded"
            : Boolean(result.value.logs?.some((line) => line.includes("exceeded CUs meter") ||
                line.includes("Computational budget exceeded")));
        if (exhaustedCompute(simulation) && !options.expectReject) {
            for (const cap of fittingCaps) {
                if (!exhaustedCompute(simulation))
                    break;
                prepared = await prepareLegs(connection, payer, pda, wsolAta, legs, total, options.slippageBps ??
                    (process.env.FORK_SLIPPAGE_BPS
                        ? Number(process.env.FORK_SLIPPAGE_BPS)
        ...
        if (simulation.value.err && !options.expectReject) {
            ...
            const flippable = prepared.filter((leg) => leg.pumpRawCurveFlagOffset !== undefined &&
                leg.jupiterData[leg.pumpRawCurveFlagOffset] === 1);
            if (attributed.isBurner && attributed.code === 6019 && flippable.length) {
                const accumulator = await ensureBareSystemPumpAccumulator(connection, payer, pda);
                if (accumulator.blockedBy) {
                    ...
                    summary.pumpVolumeTrackingDisabled = flippable.map((leg) => leg.label);
                    compiled = await compileBurn(prepared);
                    summary.txBytes = compiled.bytes;
                    if (compiled.bytes > MAX_TRANSACTION_BYTES) {
                        summary.status = "rejected";
                    ...
                    pumpCreditSnapshots = await snapshotPumpLamportCredits(connection, pda, prepared);
                    simulation = await connection.simulateTransaction(compiled.transaction, { sigVerify: true });
                }
            }
        }
        ...
        const pumpLamportCredit = pumpCreditSnapshots.reduce((totalCredit, snapshot) => totalCredit + snapshot.lamports, 0n);
        ...
        if (simulation.value.err) {
            ...
            summary.detail = [
                ...
                (simulation.value.logs ?? [])
                    .filter((line) => /Error|failed|Program log|insufficient/i.test(line))
                    .slice(-16)
                    .join("\n"),
            ]
                .filter(Boolean)
        ...
        if (confirmation.value.err) {
            ...
            const attributed = attributeFailure(landedFailure?.meta?.logMessages, confirmation.value.err);
            summary.status = "rejected";
            summary.errorCode = attributed.code;
            summary.errorName = attributed.isBurner
                ? attributed.code
        ...
        if (netSpent !== expectedNetSpent) {
            throw new Error(`vault net spent ${netSpent}, expected input ${total} minus ` +
                `validated Pump credit ${pumpLamportCredit}`);
        }
        for (const snapshot of pumpCreditSnapshots) {
            const after = await connection.getAccountInfo(snapshot.address, "confirmed");
            if (after &&
                (!after.owner.equals(web3_js_1.SystemProgram.programId) ||
                    after.data.length !== 0 ||
                    after.lamports !== 0)) {
                throw new Error(`Pump credit account ${snapshot.address.toBase58()} was not fully closed`);
            }
        }
        ...
        if (legLogs.length !== prepared.length) {
            throw new Error(`expected ${prepared.length} burn logs, saw ${legLogs.length}`);
        }
        const burned = [];
        for (const [index, leg] of prepared.entries()) {
            ...
            if (legLogs[index].amountIn !== leg.amountIn) {
                throw new Error(`${leg.label} leg input ${legLogs[index].amountIn} != expected ${leg.amountIn}`);
            }
            if (legLogs[index].burned < leg.minimumOutput) {
                throw new Error(`${leg.label} burned ${legLogs[index].burned} below signed minimum ${leg.minimumOutput}`);
            }
            const account = await (0, spl_token_1.getAccount)(connection, leg.ata, "confirmed", leg.tokenProgram);
            if (account.amount !== 0n) {
                throw new Error(`${leg.label} ATA retained ${account.amount}`);
            }
            burned.push(`${leg.label}:${legLogs[index].burned}`);
        }
        const wsol = await (0, spl_token_1.getAccount)(connection, wsolAta, "confirmed", spl_token_1.TOKEN_PROGRAM_ID);
        if (wsol.amount !== 0n) {
            throw new Error(`WSOL account retained ${wsol.amount}`);
        }
        summary.status = "burned";
        summary.burned = burned;
        summary.routes = prepared.map((leg) => `${leg.label}=${leg.routeLabel || "?"}@${leg.slippageBps}bps(${leg.slippageSource})`);
        return summary;
    }
    catch (error) {
        const code = customErrorCode(error instanceof Error ? error.message : error);
